...
def create_vit_with_localization(input_shape, patch_size, num_patches, projection_dim, transformer_units,
                                 transformer_layers, model_name, num_heads, mlp_head_units, num_classes):
    inputs = layers.Input(shape=input_shape)

    # Patch generation and encoding
    patches = Patches(patch_size)(inputs)
    encoded_patches = PatchEncoder(num_patches, projection_dim)(patches)
    ...
    features = mlp(representation, hidden_units=mlp_head_units, dropout_rate=0.5)
    class_logits = layers.Dense(num_classes, activation="softmax", name="class_output")(features)

    # Bounding box regression for localization
    bbox_output = layers.Dense(4, activation="sigmoid", name="bbox_output")(features)

    # Combine the two outputs
    model = Model(inputs=inputs, outputs=[class_logits, bbox_output], name=model_name)
    return model

# Update model_maker to include localization
def model_maker_with_localization(target_size, model_id, num_classes=3):
    ...
    # Create model with localization
    model = create_vit_with_localization(
        (*target_size, 3),
        patch_size,
        num_patches,
        projection_dim,
        transformer_units,
        transformer_layers,
        "WheatClassifier_VIT_Localization_" + str(model_id),
        num_heads,
        mlp_head_units,
        num_classes
    )
...
# Modify training to handle multiple outputs (classification and localization)
def main_with_localization():
    # Data generation
    train_gen, val_gen = gen_maker(
        train_dir,
        val_dir,
    ...
    # Model creation
    model = model_maker_with_localization((target_size, target_size), model_id)
    ...
    model.compile(
        ...
        loss={
            ...
            "bbox_output": "mean_squared_error"  # Use MSE loss for bounding box regression
        },
        metrics={
            "class_output": "acc",
            "bbox_output": "mse"
    ...
    # Train the model with both class and bounding box labels
    results = model.fit(
        train_gen,
        epochs=epochs,
        validation_data=val_gen,
    ...
    # Save history and model
    model.save(f"{backup_path}model_with_localization.h5")

# Run main function
main_with_localization()

# Prediction with localization
def predict_with_localization(model, img_path):
    ...
    bbox_coordinates = bbox_pred[0]  # Extract bounding box coordinates

    print("Predicted class:", predicted_class)
    print("Bounding box (normalized coordinates):", bbox_coordinates)
    return predicted_class, bbox_coordinates

# Load model and predict
model = tf.keras.models.load_model(
    '/content/model_with_localization.h5', 
    custom_objects={'AdamW': AdamW}
)
img_path = '/content/drive/MyDrive/plant/val/Brown_rust/Brown_rust036.jpg'
predict_with_localization(model, img_path)